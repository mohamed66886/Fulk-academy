...
export function AttendanceScanner({
  ...
  // 1. Unified Process Scan Handler (No code repetition)
  const processScan = React.useCallback(
    (token: string, source: "camera" | "scanner") => {
      ...
      if (source === "scanner") {
        // No-op for now since UI doesn't display it
      }
    ...
    async function checkCameras() {
      try {
        ...
        if (isMounted && devices && devices.length > 0) {
          ...
          setSelectedCameraId(backCam ? backCam.id : firstCam ? firstCam.id : "");
        }
      } catch {
          ...
          <div className="relative overflow-hidden rounded-2xl bg-black shadow-inner">
            ...
            {isCameraActive && (
              <>
                ...
                <div
                  className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 rounded-full bg-black/50 px-4 py-2 text-xs font-bold text-white backdrop-blur-md border border-white/10"
                  dir="rtl"
                >
            ...
            {!isCameraActive && !cameraError && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-surface-raised p-6 text-center">
                ...
                <Button
                  onClick={() => startCamera()}
                  className="gap-2 rounded-xl px-8 shadow-lg font-bold"
                  dir="rtl"
                >
            ...
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted max-w-sm mx-auto leading-relaxed">
                  قم بتوجيه مسدس الباركود نحو كارت الطالب واضغط الزر. سيتم رصد الكود وتثبيت الحضور
                  فوراً وبدون أي تدخل إضافي.
                </p>
              </div>
